    ...
    def render_charts(event=None):
        try:
            ...
            time_axis, rate_paths = simulate_vasicek(
                r0, a, b, sigma, T=T, num_sims=num_sims
            )
            expected_path = calculate_expected_path(r0, a, b, T=T)
            config_sim = create_simulation_chart(
                time_axis, rate_paths, expected_path, num_sims, b, T
            )
            ...
            # 2. Yield Curve
            yc_maturities, yc_yields = calculate_yield_curve(
                r0, a, b, sigma, max_maturity=T
            )
            ...
            document.getElementById(
                "metric-conf"
            ).innerText = f"{(mu - 1.96 * std):.2%} - {(mu + 1.96 * std):.2%}"
