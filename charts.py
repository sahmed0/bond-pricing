...
def create_simulation_chart(
    time_axis, rate_paths, expected_path, num_sims, long_term_mean, time_horizon
):
    """
        ...
        data.append(
            {
                ...
                "y": rate_paths[:, i].tolist()
                if isinstance(rate_paths, np.ndarray)
                else rate_paths[:, i],
                "mode": "lines",
                "opacity": 0.4,
...
def create_yield_curve_chart(maturities, yields):
    ...
    data = [
        {
            "x": maturities.tolist()
            if isinstance(maturities, np.ndarray)
            else maturities,
